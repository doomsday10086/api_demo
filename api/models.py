...
class Account(models.Model):
    username = models.CharField("用户名", max_length=64, unique=True, null=True, blank=True)
    ...
    class Meta:
        verbose_name_plural = "01. 账户信息"

    def save(self, *args, **kwargs):
        if not self.pk:
            # This code only happens if the objects is not in the database yet. Otherwise it would have pk
            m = hashlib.md5()
            m.update(str(self.mobile).encode(encoding="utf-8"))
            self.uid = m.hexdigest()
            self.username = 'yk' + str(self.mobile)
...
class UserAuthToken(models.Model):
    ...
    class Meta:
        verbose_name_plural = "02. 用户Token"
    ...
    def generate_key(self):
        ...
        md5 = hashlib.md5(str(mobile).encode('utf-8'))
        md5.update(now)
...
class OftenAskedQuestion(models.Model):
    ...
    content_type = models.ForeignKey(ContentType,
                                     limit_choices_to={'model__contains': 'course'},
...
class Question(models.Model):
    ...
    degree_course = models.ForeignKey("DegreeCourse", blank=True, null=True,
                                      on_delete=models.CASCADE)  # 若是针对整个学位课程的提问，关联这个
    course_section = models.ForeignKey("CourseSection", blank=True, null=True,
                                       on_delete=models.CASCADE)  # 针对整个学位课程的提问不需关联特定课时
